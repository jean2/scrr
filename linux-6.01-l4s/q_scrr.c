/*
 * q_scrr.c		Parse/print SCRR discipline module options.
 *
...
{
	fprintf(stderr,
		"Usage: ... scrr [ limit PACKETS ] [ buckets NUMBER ] [ hash_mask MASK ]\n"
		"                [ flow_limit PACKETS ]\n");
}
...
static int scrr_parse_opt(struct qdisc_util *qu,
			  int argc,
			  char **argv,
			  struct nlmsghdr *n,
			  const char *dev)
{
	...
	if (plimit != 0xFFFFFFFF)
		addattr32(n, 1024, TCA_SCRR_PLIMIT, plimit);
	if (buckets != 0) {
		unsigned int hash_log = ilog2(buckets);
		addattr32(n, 1024, TCA_SCRR_BUCKETS_LOG, hash_log);
	}
	if (hash_mask != 0x0)
		addattr32(n, 1024, TCA_SCRR_HASH_MASK, hash_mask);
	if (flow_plimit != 0xFFFFFFFF)
		addattr32(n, 1024, TCA_SCRR_FLOW_PLIMIT, flow_plimit);
	if (flags_upd)
		addattr32(n, 1024, TCA_SCRR_FLAGS, flags);
...
static int scrr_print_opt(struct qdisc_util *qu,
			  FILE *f,
			  struct rtattr *opt)
{
	struct rtattr *tb[TCA_SCRR_MAX + 1];

	if (opt == NULL)
		return 0;

	parse_rtattr_nested(tb, TCA_SCRR_MAX, opt);

	if (tb[TCA_SCRR_PLIMIT] &&
	    RTA_PAYLOAD(tb[TCA_SCRR_PLIMIT]) >= sizeof(__u32)) {
		unsigned int plimit;
		plimit = rta_getattr_u32(tb[TCA_SCRR_PLIMIT]);
		print_uint(PRINT_ANY, "limit", "limit %up ", plimit);
	}

	if (tb[TCA_SCRR_BUCKETS_LOG] &&
	    RTA_PAYLOAD(tb[TCA_SCRR_BUCKETS_LOG]) >= sizeof(__u32)) {
		unsigned int buckets_log;
		buckets_log = rta_getattr_u32(tb[TCA_SCRR_BUCKETS_LOG]);
		print_uint(PRINT_ANY, "buckets", "buckets %u ",
			   1U << buckets_log);
	}

	if (tb[TCA_SCRR_HASH_MASK] &&
	    RTA_PAYLOAD(tb[TCA_SCRR_HASH_MASK]) >= sizeof(__u32)) {
		unsigned int hash_mask;
		hash_mask = rta_getattr_u32(tb[TCA_SCRR_HASH_MASK]);
		print_uint(PRINT_ANY, "hash_mask", "hash_mask %u ",
			   hash_mask);
	}

	if (tb[TCA_SCRR_FLOW_PLIMIT] &&
	    RTA_PAYLOAD(tb[TCA_SCRR_FLOW_PLIMIT]) >= sizeof(__u32)) {
		unsigned int flow_plimit;
		flow_plimit = rta_getattr_u32(tb[TCA_SCRR_FLOW_PLIMIT]);
		print_uint(PRINT_ANY, "flow_limit", "flow_limit %up ",
			   flow_plimit);
	}

	if (tb[TCA_SCRR_FLAGS] &&
	    RTA_PAYLOAD(tb[TCA_SCRR_FLAGS]) >= sizeof(__u32)) {
		unsigned int flags;
		flags = rta_getattr_u32(tb[TCA_SCRR_FLAGS]);
		print_uint(PRINT_ANY, "flags", "flags 0x%X ", flags);
	}
	return 0;
}

static int scrr_print_xstats(struct qdisc_util *qu, FILE *f,
...
struct qdisc_util scrr_qdisc_util = {
	.id = "scrr",
	.parse_qopt = scrr_parse_opt,
	.print_qopt = scrr_print_opt,
	.print_xstats = scrr_print_xstats,
};

struct qdisc_util scrr_npm_qdisc_util = {
	.id = "scrr_npm",
	.parse_qopt = scrr_parse_opt,
	.print_qopt = scrr_print_opt,
	.print_xstats = scrr_print_xstats,
...
struct qdisc_util scrr_nmia_qdisc_util = {
	.id = "scrr_nmia",
	.parse_qopt = scrr_parse_opt,
	.print_qopt = scrr_print_opt,
	.print_xstats = scrr_print_xstats,
...
struct qdisc_util scrr_nmne_qdisc_util = {
	.id = "scrr_nmne",
	.parse_qopt = scrr_parse_opt,
	.print_qopt = scrr_print_opt,
	.print_xstats = scrr_print_xstats,
...
struct qdisc_util scrr_neia_qdisc_util = {
	.id = "scrr_neia",
	.parse_qopt = scrr_parse_opt,
	.print_qopt = scrr_print_opt,
	.print_xstats = scrr_print_xstats,
...
struct qdisc_util scrr_basic_qdisc_util = {
	.id = "scrr_basic",
	.parse_qopt = scrr_parse_opt,
	.print_qopt = scrr_print_opt,
	.print_xstats = scrr_print_xstats,
